...
console.log("Script versie: 4.1 - Definitieve Tooltip Fix");
...
window.COACH_TOOLS = {
    exportCoachDataForCorrection: async function() {
        ...
        coachesSnapshot.forEach(doc => {
            const data = doc.data();
            coaches.push({ id: doc.id, naam: data.naam, foto_url: data.foto_url || '' });
        });
    ...
    updateCoachPhotosInFirestore: async function(correctedData) {
        if (!window.db) return console.error("DB niet klaar.");
        if (!correctedData || !Array.isArray(correctedData)) return alert("Geen correcte data meegegeven.");
        ...
        try {
            await batch.commit();
            alert("Alle foto-URLs zijn bijgewerkt! Herlaad de pagina.");
        } catch (error) {
    ...
    fixBritishFlagCodes: async function() {
        ...
        const [engSnapshot, sctSnapshot, wlsSnapshot] = await Promise.all([getDocs(qEng), getDocs(qSct), getDocs(qWls)]);
        ...
        try {
            ...
            if (total > 0) alert(`${total} vlag-codes succesvol gecorrigeerd! Herlaad de pagina.`);
            else alert("Geen incorrecte vlag-codes gevonden om te corrigeren.");
        } catch (error) {
...
function drawHeatmap(data) {
    ...
    const svg = d3.select("#heatmap-container").append("svg").attr("width", width + margin.left + margin.right).attr("height", height + margin.top + margin.bottom).append("g").attr("transform", `translate(${margin.left}, ${margin.top})`);
    ...
    svg.append("g").attr("class", "axis").attr("transform", `translate(0, ${height})`).call(xAxis).selectAll("text").style("text-anchor", "end").attr("dx", "-.8em").attr("dy", "-.5em").attr("transform", "rotate(-90)");
    const yAxisGroup = svg.append("g").attr("class", "axis y-axis");
    const yAxisTicks = yAxisGroup.selectAll(".tick").data(logoData).enter().append("g").attr("class", "tick").attr("transform", d => `translate(0, ${y(d.Club) + y.bandwidth() / 2})`);
        ...
        if (hasPhoto) {
            tooltip.append('img')
                ...
                .on('error', function() {
                    ...
                    tooltip.insert('svg', ':first-child').attr('class', 'tooltip-img').attr('viewBox', '0 0 50 50').append('path').attr('d', avatarIconPath).attr('fill', '#ccc');
                });
        } else {
            tooltip.append('svg').attr('class', 'tooltip-img').attr('viewBox', '0 0 50 50').append('path').attr('d', avatarIconPath).attr('fill', '#ccc');
        }
    ...
    svg.selectAll(".prize-group").data(data.filter(d => d.landstitel === 'Y' || d.nationale_beker === 'Y' || d.europese_prijs === 'Y')).enter().append("g").attr("class", "prize-group").attr("transform", d => `translate(${x(d.seizoen) + x.bandwidth() / 2}, ${y(d.club) + y.bandwidth() / 2})`).each(function(d) {
